...
const ChatWidget = () => {
  ...
  useEffect(() => {
    if (isClient && isExpanded) {
      setUnreadCount(0); // Clear unread count when expanded
    }
  }, [isExpanded, isClient]);

  useEffect(() => {
    if (isClient && !isExpanded) {
      // Count bot messages that arrived while minimized
      const newUnreadCount = messages.filter(msg =>
        msg.sender === 'bot' &&
        msg.timestamp > new Date(Date.now() - 1000) // Messages from last second
      ).length;
      if (newUnreadCount > 0) {
        setUnreadCount(prev => prev + newUnreadCount);
      }
    }
  }, [messages, isExpanded, isClient]);
  ...
  // Render minimized chat icon when not expanded
  if (!isExpanded) {
    return (
      <div className={styles.chatIcon} onClick={() => setIsExpanded(true)}>
        <div className={styles.chatIconContent}>
  ...
  // Render full chat widget when expanded
  return (
